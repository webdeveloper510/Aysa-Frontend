 ...
 const [apiData, setApiData] = useState(null); // store API response
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post("https://api.the-aysa.com/product-semantic-search", {
          query: "Nike Air Jordan 5",
        });
  ...
  return (
    <>
      ...
      {loading && <Typography align="center">Loading data...</Typography>}
      ...
      {!loading && !error && (
        <>
          ...
          <Box display="flex" flexWrap="wrap" justifyContent="space-between" my={4} mb={5}>
