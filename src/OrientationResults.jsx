...
export default function OrientationResults(props) {
  ...
  const [screenOrientation, setScreenOrientation] = useState('landscape');
    ...
    if(item.price.includes("-")){
      number1 = item.price.split('-')[0].substring(1, item.price.length).replaceAll(",","");
      number2 = item.price.split('-')[1].substring(1, item.price.length).replaceAll(",","");
    } else {
      number2 = item.price.substring(1, item.price.length).replaceAll(",","");
    }
    console.log(number2)
    let profit = Number((item.profitmargin / 100 * Number(number2.replaceAll("$", ""))).toFixed(2));
    let production = Number((Number(number2.replaceAll("$", "")) - profit).toFixed(2));
